...
response = WS.sendRequest(findTestObject('Insert Posts', [('Title') : Title, ('Body') : Body, ('user_id') : user_id]))

if (WS.verifyResponseStatusCode(response, 201, FailureHandling.OPTIONAL)) {
    response2 = WS.sendRequest(findTestObject('Get Posts', [('user_id') : user_id]))
